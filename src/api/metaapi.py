...
def generate_ra_search(identifier: str) -> Tuple[str]:
    ...
    if scheme == "omid":
        return (
            "<https://w3id.org/oc/meta/{0}> ^pro:isHeldBy ?knownRole.".format(
                literal_value
            ),
        )
    else:
    ...
    for result in results[1:]:
        ...
        for i, data in enumerate(result):
            if i == header.index("type"):
                ...
                i == header.index("author")
                or i == header.index("editor")
                or i == header.index("publisher")
            ):
...
def process_ordered_list(items):
    ...
    start_role = next(
        iter(
            role
            for role, next_role in items_dict.items()
            if role not in items_dict.values()
        )
    )
