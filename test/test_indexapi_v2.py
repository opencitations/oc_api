...
def test_citation_count_zero(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, "/index/v2/citation-count/omid:br/06011163661")
    )
...
def test_reference_count(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, f"/index/v2/reference-count/{MAIN_PAPER_OMID}")
    )
...
def test_reference_count_zero(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, "/index/v2/reference-count/omid:br/06103007140")
    )
    assert result == [{"count": "0"}]
...
def test_references(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, f"/index/v2/references/{MAIN_PAPER_OMID}")
    )
...
def test_references_no_outgoing(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, "/index/v2/references/omid:br/06103007140")
    )
    assert result == []
...
def test_venue_citation_count(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, "/index/v2/venue-citation-count/issn:2641-3337")
    )
...
def test_citation_count_zenodo_dmp(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, f"/index/v2/citation-count/{ZENODO_DMP_OMID}")
    )
...
def test_citations_with_author_sc(api_manager: APIManager) -> None:
    result = json.loads(
        execute_operation(api_manager, f"/index/v2/citations/{QSS_ARTICLE_OMID}")
    )
    assert normalize_citations(result) == normalize_citations(
        EXPECTED_QSS_ARTICLE_CITATIONS
    )
